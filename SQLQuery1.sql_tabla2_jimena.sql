...
insert into Profe(doc_prof,nom_prof,ape_prof,cate_prof,sal_prof)
values
...
from Estudiante where edad_est in(select min(edad_est)from Estudiante);

select avg (valor_cur) 
from curso where horas_cur > 40; 

select avg (sal_prof) 
from Profe where cate_prof = 1;

select *
from curso order by valor_cur;

select nom_prof 
from Profe where sal_prof in(select min(sal_prof)from Profe);
