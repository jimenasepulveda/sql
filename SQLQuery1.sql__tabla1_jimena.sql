...
select *
from compañia where comañofun >= 1991 and comañofun <= 1998;

select autoplaca, automodelo, automarca, autopasajeros, autocilindraje, autnombre, asecosto, asevalorasegurado
from automotores, aseguramientos, tiposAutomotores where autoplaca= aseplaca and autotipo=autotipo and asefechaexpiracion >='2013-09-30';

select incifecha, inciplaca, incilugar, asecodigo, asefechainicio, asevalorasegurado
from incidentes, aseguramientos where inciplaca=aseplaca and incifecha ='2012-09-30'; 

select inciantheridos, autoplaca, incilugar, incifecha, asefechainicio, asecosto, aseestado,asevalorasegurado
from incidentes, aseguramientos, automotores where inciplaca= aseplaca and autoplaca=inciplaca and inciantheridos=1;

select * 
from aseguramientos where asecosto in(select max(asecosto)from aseguramientos);

select incicanautosinvolucrados, inciplaca, incifecha, asevalorasegurado, aseestado
from incidentes, aseguramientos where inciplaca=aseplaca and incicanautosinvolucrados in(select min(incicanautosinvolucrados)from incidentes);

select inciplaca, incifecha, incilugar, inciantheridos, asefechainicio, asefechaexpiracion, asevalorasegurado
from incidentes, aseguramientos where inciplaca= aseplaca and inciplaca= 'FLL420';

select *
from compañia where comnit = '899999999-5';

select * 
from aseguramientos, automotores where aseplaca= autoplaca and asevalorasegurado in(select max (asevalorasegurado)from aseguramientos);
